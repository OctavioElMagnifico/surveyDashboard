---
title: "Coordenadas Discriminantes"
...
dataF <- dataF %>% mutate( dc1 = ( cdRFP$proyecciones )$a1, dc2 = ( cdRFP$proyecciones )$a2 )

dataF <- dataF %>% select( ID,dc1,dc2,organic, 39,65,19,20,18,66 )


write_csv(x=dataF, path = "../output/discriminantCoordinates.csv" )

centroideT <- dataF %>% filter( organic == TRUE ) %>% select( -organic, -ID ) %>% colMeans %>% enframe() %>% spread(name,value) %>% mutate( ID="centroid", organic = TRUE )

centroideF <- dataF %>% filter( organic == FALSE ) %>% select( -organic, -ID ) %>% colMeans %>% enframe() %>% spread(name,value) %>% mutate( ID="centroid", organic = FALSE )

rbind( dataF, centroideF, centroideT ) %>% write_csv( path = "../output/discriminantCoordinates.csv" )
...
centroides %>% write_csv( path="../output/discriminantCentroids.csv" )
...
componentsNames <- cdRFP$matrizB %>% rownames
components1 <-   cdRFP$listaVectores[[1]]
components2 <-   cdRFP$listaVectores[[2]]

names( components1 ) <- componentsNames
names( components2 ) <- componentsNames

tibble(
  component = componentsNames,
  amount = components1
) %>% write.csv( file="../output/discriminantC1.csv" )

tibble(
  component = componentsNames,
  amount = components2
) %>% write.csv( file="../output/discriminantC2.csv" )
